...
// ══ Fix 1: 更新 Footer 日期 ══
const rDate1 = /(🕐\s*最近更新：)\d+年\d+月\d+日(</span>)/;
const rDate2 = /(📅\s*下次更新：)\d+年\d+月\d+日（[^）]+）(</span>)/;

if (!rDate1.test(content)) console.log('WARNING: 最近更新 pattern not found');
else content = content.replace(rDate1, '$1' + '2026年6月2日' + '$2');

if (!rDate2.test(content)) console.log('WARNING: 下次更新 pattern not found');
else content = content.replace(rDate2, '$1' + '2026年6月9日（周二）' + '$2');

console.log('✅ Fix 1: Footer 日期已更新');
...
// 用正则把 class="tab-btn active" + p6 替换为 class="tab-btn" + p6
const rP6Tab = /(<button class="tab-btn) active(" onclick="switchTab\(this,'p6'\)">📅 2026\.5\.19–5\.25<\/button>)/;
