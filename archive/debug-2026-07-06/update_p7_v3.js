...
// ═══ Edit 1: PERIODS 数组头部新增 p7 ═══
// 策略：匹配 "  { id: 'p6'" 所在行，在其前面插入 p7 行
const lines = content.split('\n');
let p6LineIdx = -1;
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes("{ id: 'p6'") && lines[i].includes('label')) {
    p6LineIdx = i;
    break;
  }
}
if (p6LineIdx === -1) {
  console.log('ERROR: p6 PERIODS line not found!');
  process.exit(1);
}
const p7Line = "  { id: 'p7',  label: '2026.5.26–6.1',  start: new Date(2026, 4, 26), end: new Date(2026, 5,  1) },";
lines.splice(p6LineIdx, 0, p7Line);
content = lines.join('\n');
console.log('✅ Edit 1: PERIODS 数组已新增 p7 (at line ' + (p6LineIdx + 1) + ')');
ok++;

// ═══ Edit 2: Tabs 新增 p7 并设为 active ═══
// 找到 p6 tab 按钮所在行，插入 p7 tab 行，并去掉 p6 的 active class
const lines2 = content.split('\n');
let p6TabIdx = -1;
for (let i = 0; i < lines2.length; i++) {
  if (lines2[i].includes("switchTab(this,'p6')") && lines2[i].includes('tab-btn')) {
    p6TabIdx = i;
    break;
  }
}
if (p6TabIdx === -1) {
  console.log('ERROR: p6 tab line not found!');
  process.exit(1);
}
// 去掉 p6 tab 的 active class
lines2[p6TabIdx] = lines2[p6TabIdx].replace('tab-btn active', 'tab-btn');
// 在 p6 tab 前面插入 p7 tab（active）
const p7Tab = '  <button class="tab-btn active" onclick="switchTab(this,\'p7\')">📅 2026.5.26–6.1</button>';
lines2.splice(p6TabIdx, 0, p7Tab);
content = lines2.join('\n');
console.log('✅ Edit 2: Tabs 已新增 p7 (active) 并移除 p6 active');
ok++;

// ═══ Edit 3: 新增 p7 period-content HTML 块（插入在 p6 块之前） ═══
// 找到 "<!-- ════ 期次6" 标记行，在其前面插入 p7 HTML
const marker = '<!-- ════ 期次6：';
const p7Idx = content.indexOf(marker);
if (p7Idx === -1) {
  console.log('ERROR: p6 HTML marker not found!');
  process.exit(1);
}
...
content = content.slice(0, p7Idx) + p7Html + '\n' + content.slice(p7Idx);
console.log('✅ Edit 3: p7 period-content HTML 块已插入（在 p6 块之前）');
ok++;

// ═══ Edit 4: 更新 Footer 日期 ═══
// 用正则匹配日期，不依赖精确 emoji/dash 字符
const reFooter1 = /(🕐\s*最近更新：)\d+年\d+月\d+日(<\/span>)/;
const reFooter2 = /(📅\s*下次更新：)\d+年\d+月\d+日（[^）]+）(<\/span>)/;

if (!reFooter1.test(content)) {
  console.log('WARNING: 最近更新 pattern not found');
} else {
  content = content.replace(reFooter1, '$1' + '2026年6月2日' + '$2');
  console.log('✅ Edit 4a: 最近更新 → 2026年6月2日');
}
if (!reFooter2.test(content)) {
  console.log('WARNING: 下次更新 pattern not found');
} else {
  content = content.replace(reFooter2, '$1' + '2026年6月9日（周二）' + '$2');
  console.log('✅ Edit 4b: 下次更新 → 2026年6月9日（周二）');
}
ok++;

// ═══ Edit 5: 更新 activePeriodId ═══
if (content.includes("let activePeriodId = 'p5'")) {
  content = content.replace("let activePeriodId = 'p5'", "let activePeriodId = 'p7'");
  console.log('✅ Edit 5: activePeriodId → p7 (was p5)');
} else if (content.includes("let activePeriodId = 'p6'")) {
  content = content.replace("let activePeriodId = 'p6'", "let activePeriodId = 'p7'");
  console.log('✅ Edit 5: activePeriodId → p7 (was p6)');
} else if (content.includes("let activePeriodId = 'p7'")) {
  console.log('✅ Edit 5: activePeriodId already p7');
} else {
  console.log('WARNING: activePeriodId not found!');
}
ok++;
